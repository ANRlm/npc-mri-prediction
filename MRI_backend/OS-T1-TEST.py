...
# 保存模型和重要的性能指标
# 创建保存目录（如果不存在）
os.makedirs('模型构建/OS/OS-T1/models', exist_ok=True)

# 保存模型
with open('模型构建/OS/OS-T1/models/adasyn_cox_model.pkl', 'wb') as f:
    pickle.dump(estimator, f)

# 保存标准化器
with open('模型构建/OS/OS-T1/models/scaler.pkl', 'wb') as f:
...
with open('模型构建/OS/OS-T1/models/adasyn_model_info.pkl', 'wb') as f:
    pickle.dump(model_info, f)

print("\nADASYN过采样Cox模型、标准化器和性能指标已保存到 '模型构建/OS/OS-T1/models/' 目录")
...
try:
    ...
    # 保存加权模型
    with open('模型构建/OS/OS-T1/models/weighted_cox_model.pkl', 'wb') as f:
...
def compare_all_models(models_dict, X_test_data, y_test_data, title="所有模型性能对比"):
    ...
    # 保存结果
    os.makedirs('模型构建/OS/OS-T1', exist_ok=True)
    perf_df.to_csv('模型构建/OS/OS-T1/all_models_comparison.csv', index=False)
